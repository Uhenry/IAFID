...
{
    public partial class Fintran_Review : Form
    {
        ...
        private Boolean batchDataLoaded = false;
        ...
        public Fintran_Review()
        {
            InitializeComponent();
        }

        private void Fintran_Review_Load(object sender, EventArgs e)
        {
            ...
            if (!UserInfo.userRole.Equals("Administrator"))
            {
                this.BatchReviewWindow.Hide();
            }
            else
            {
        ...
        {
            this.ListSelectBatch.DataSource = dbcon.getBatchUserInfo();
            foreach (GridViewDataColumn column in this.ListSelectBatch.MultiColumnComboBoxElement.Columns)
            {
                column.BestFit();
            }
        }
        ...
        public void loadReportNumbers(string BatchNumber)
        {
            //Loads All Reports ID related to selected Batch
            this.listReportNumber.DataSource = dbcon.getReportNumber2(BatchNumber);
            this.listReportNumber.DisplayMember = "ReportID_";
        ...
        private void ListSelectBath_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            //this.Text = "Insitution:  " + this.txtReportFIStepOne.Text.ToString().Trim() + " Batch Number: " + this.ListSelectBathStepOne.SelectedValue.ToString();
            //this.Report_Window.Text = "Report Main Menu - Batch Number:  " + this.ListSelectBathStepOne.SelectedValue.ToString();
            ////Loads Branch Names
            //loadBranchNames(this.txtReportFIStepOne.Text.ToString().Trim());
        }

        //Load All Branch Related Info
        private void listBranchNameStepOne_SelectedValueChanged(object sender, EventArgs e)
        {
            if (this.chkBranchNameInfo.Checked)
            {
                loadBranchInfo(this.txtReportFIStepOne.Text.ToString().Trim(), Utility.ReplaceSingleQuote(((DataRowView)this.listBranchNameStepOne.SelectedItem)["branchname_"].ToString()));
            }
        }
        ...
        private void listReportTypeStepOne_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                if (this.listReportTypeStepOne.Text.ToString() == "Select")
                {
                    // Cancel the event
                    e.Cancel = true;
        ...
        //Load Save Step One Information
        private void btnSearchStepOne_Click(object sender, EventArgs e)
        {
            ReportCompleted();

            switch (this.btnSearchStepOne.Text)
            {
                case "Search":

                    if (!string.IsNullOrWhiteSpace(this.txtReportNumberStepOne.Text.ToString().Trim()))
                    {
                        ReadOnlyButtons(true);
                        this.chkBranchNameInfo.Checked = false;
                        if (UserInfo.userRole.Equals("Administrator"))
                        {
                            LoadStepOneInformation(this.txtReportNumberStepOne.Text.ToString().Trim());
                        }
                        else //Data Entry
                        {
                            Boolean result = dbcon.ReportGracePeriod(this.txtReportNumberStepOne.Text.ToString().Trim());
                            if (result)
                            {
                                LoadStepOneInformation(this.txtReportNumberStepOne.Text.ToString().Trim());
                                this.chkBoxReportNumberUpdate.Visible = true;
                            }
                            else
                            {
                                MessageBox.Show("Your Grace Period for this report has expired Please Contact Your Supervisor.", "Grace Period - Read Only Functions Enabled", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                LoadStepOneInformation(this.txtReportNumberStepOne.Text.ToString().Trim());
                                ReadOnlyButtons(false);
                                this.chkBoxReportNumberUpdate.Visible = false;
                                this.chkBoxReportNumberUpdate.Checked = false;
                            }
                        }

                        dbcon.AuditUser(UserInfo.userName, "Report Search - Criteria = Report ID " + FintranList.ReportID, 2, DateTime.Now);
                    }
                    else
                    {
                        MessageBox.Show("Report Number Required", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    break;

                case "Update Report Number":
                    if (MessageBox.Show("Are you sure you want to change Report Number FROM - TO:\nCurrent ReportID: " + FintranList.ReportID + "\nNew ReportID: " + this.txtReportNumberStepOne.Text.ToString(), "Confirm ReportID Change", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        try
                        {
                            Boolean result = dbcon.ChangeReport(FintranList.ReportID, this.txtReportNumberStepOne.Text.ToString());
                            if (result)
                            {
                                dbcon.AuditUser(UserInfo.userName, "Report ID Updated - OLD = " + FintranList.ReportID + " NEW = " + this.txtReportNumberStepOne.Text.ToString(), 2, DateTime.Now);
                                MessageBox.Show("Report Number Updated Successfully", "Sucess", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                ReportCompleted();
                                this.btnSearchStepOne.Text = "Search";
                                this.chkBoxReportNumberUpdate.Checked = false;
                                this.chkBoxReportNumberUpdate.Visible = false;
                            }
                        }
        ...
        public void LoadStepOneInformation(String reportID)
        {
            FintranList.ReportID = reportID;

            SqlDataReader rd = dbcon.getStepOneInformation(FintranList.ReportID);
            if (rd.HasRows)
            {
                while (rd.Read())
                {
                    this.txtBatchStepOne.Text = (object)rd["Batch_Number"] == DBNull.Value ? string.Empty : (string)rd["Batch_Number"];
                    this.listReportTypeStepOne.Text = (object)rd["Report_type"] == DBNull.Value ? string.Empty : (string)rd["Report_type"];
                    this.txtReportFIStepOne.Text = (object)rd["Instition_Name"] == DBNull.Value ? string.Empty : (string)rd["Instition_Name"];
                    this.txtInsitutionTypeStepOne.Text = (object)rd["Institution_Type"] == DBNull.Value ? string.Empty : (string)rd["Institution_Type"];
                    this.txtTRNStepOne.Text = (object)rd["TRN_"] == DBNull.Value ? string.Empty : (string)rd["TRN_"];
                    this.txtAddressStepOne.Text = (object)rd["Inst_address"] == DBNull.Value ? string.Empty : (string)rd["Inst_address"]; ;
                    this.txtBranchAddressStepOne.Text = (object)rd["Branch_Address"] == DBNull.Value ? string.Empty : (string)rd["Branch_Address"];
                    this.txtPrepFNameStepOne.Text = (object)rd["Preparer_FirstName"] == DBNull.Value ? string.Empty : (string)rd["Preparer_FirstName"];
                    this.txtPrepLNameStepOne.Text = (object)rd["Preparer_LastName"] == DBNull.Value ? string.Empty : (string)rd["Preparer_LastName"];
                    this.txtPrepMIStepOne.Text = (object)rd["Preparer_MI"] == DBNull.Value ? string.Empty : (string)rd["Preparer_MI"];
                    this.txtPrepTitleStepOne.Text = (object)rd["Preparer_Title"] == DBNull.Value ? string.Empty : (string)rd["Preparer_Title"];
                    this.txtPrepPhoneStepOne.Text = (object)rd["PreparerPhone_Number"] == DBNull.Value ? string.Empty : (string)rd["PreparerPhone_Number"];

                    if ((object)rd["Preparer_Signature_date"] != DBNull.Value)
                    {
                        this.dtimePrepSignedStepOne.Value = (DateTime)rd["Preparer_Signature_date"];
                    }
                    else
                    {
                        this.dtimePrepSignedStepOne.Value = null;
                    ...
                    this.txtContLNameStepOne.Text = (object)rd["Contact_LastName"] == DBNull.Value ? string.Empty : (string)rd["Contact_LastName"];
                    this.txtContFNameStepOne.Text = (object)rd["Contact_FirstName"] == DBNull.Value ? string.Empty : (string)rd["Contact_FirstName"];
                    this.txtContMIStepOne.Text = (object)rd["Contact_MI"] == DBNull.Value ? string.Empty : (string)rd["Contact_MI"];
                    this.txtContTitleStepOne.Text = (object)rd["Contact_Title"] == DBNull.Value ? string.Empty : (string)rd["Contact_Title"];
                    this.txtContPhoneStepOne.Text = (object)rd["ContactPhone_Number"] == DBNull.Value ? string.Empty : (string)rd["ContactPhone_Number"];

                    //STR Reason If STR
                    this.txtSTRStepFour.Text = (object)rd["Susp_Trans_Info"] == DBNull.Value ? string.Empty : (string)rd["Susp_Trans_Info"];
                }
            ...
            {
                MessageBox.Show("Report Number Not Found, Please Try Again or Contact System Administrator", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ReportCompleted();
            }
        ...
        //Load All Branch Related Info
        private void chkBranchNameInfo_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (this.chkBranchNameInfo.Checked)
            {
                loadBranchInfo(this.txtReportFIStepOne.Text.ToString().Trim(), Utility.ReplaceSingleQuote(((DataRowView)this.listBranchNameStepOne.SelectedItem)["branchname_"].ToString()));
        ...
        private void btnSave_UpdateStepOne_Click(object sender, EventArgs e)
        {
            ...
            Boolean result = dbcon.UpdateTransactionReport(this.listReportTypeStepOne.Text.ToString(),
           ...
           this.txtBatchStepOne.Text.ToString(), this.txtInsitutionTypeStepOne.Text.ToString());
            ...
            if (result)
            {
                ...
                dbcon.AuditUser(UserInfo.userName, "PART 1 Record Updated - Criteria = Report ID " + FintranList.ReportID, 2, DateTime.Now);
            }
            else
            {
        ...
        public void ClearStepOne()
        {
            ...
            this.txtReportFIStepOne.Text = string.Empty;
                ...
                case "Update":
                    ...
                    Boolean Updateresult = dbcon.UpdatePerson(FintranList.UniqueID, this.ListInvolvementStepTwo.Text.ToString(),
                        ...
                        this.txtTRNStepTwo.Text.ToString().Trim(), this.listIDMethodStepTwo.Text.ToString(),
                         idcred, issued, DOB,
                        this.txtIDNumStepTwo.Text.ToString().Trim(), this.txtAccountNumberStepTwo.Text.ToString(),
                        this.listAccountTypeStepTwo.Text.ToString(), this.txtOccupationStepTwo.Text.ToString().Trim());

                    if (Updateresult)
                    {
                        dbcon.AuditUser(UserInfo.userName, "PART 2 Record Updated - Criteria = Report ID " + FintranList.ReportID + " and UniqueID = " + FintranList.UniqueID, 2, DateTime.Now);
                        this.btnSaveStepTwo.Enabled = false;
                        this.btnNewRecordStepTwo.Enabled = true;
        ...
        private void btnDeleteStepTwo_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Are you sure you want to delete this Record ?", "Delete", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    ...
                    if (result)
                    {
                        dbcon.AuditUser(UserInfo.userName, "PART 2 Record Deleted - Criteria = Report ID " + FintranList.ReportID, 2, DateTime.Now);
        ...
        private void btnSave_UpdateStepThree_Click(object sender, EventArgs e)
        {
            ...
            switch (this.btnSave_UpdateStepThree.Text)
            {
                ...
                case "Update":
                    ...
                    if (Updateresult)
                    {
                        dbcon.AuditUser(UserInfo.userName, "PART 3 Record Updated - Criteria = Report ID " + FintranList.UniqueID, 2, DateTime.Now);
                        this.btnSave_UpdateStepThree.Enabled = false;
                        this.btnNewRecordStepThree.Enabled = true;
        ...
        public void ClearStepThree()
        {
            ...
            //listSelectAgentStepThree.SelectedValue = 0;
        ...
        private void SetUpGrid()
        {
            ...
            gridViewControl.Columns.Add(new GridViewTextBoxColumn("Involvement__Type"));
        ...
        private void btnDeleteRecordStepThree_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Are you sure you want to delete this Record ?", "Delete", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    ...
                    if (result)
                    {
                        dbcon.AuditUser(UserInfo.userName, "PART 3 Record Deleted - Criteria = Report ID " + FintranList.ReportID, 2, DateTime.Now);
        ...
        private void btnSave_UpdateStepFour_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrWhiteSpace(this.txtSTRStepFour.Text.ToString()))
            {
                ...
                if (result)
                {
                    dbcon.AuditUser(UserInfo.userName, "PART 4 Record Updated - Criteria = Report ID " + FintranList.ReportID, 2, DateTime.Now);
                    this.btnCompletedStepFour.Visible = true;
                    this.lblStatusStepFour.Text = "Report Number: " + FintranList.ReportID + " - Record Updated Sucessfully";
        ...
        //Load STEP TWO AND STEP THREE DATA IN GRID
        private void Report_radPageView_SelectedPageChanged(object sender, EventArgs e)
        {
            ...
            //If STEP SELECTED LOAD PERSON DETAILS
            if (this.Report_radPageView.SelectedPage == this.StepTwo_PageViewOuter)
            {
                loadGridViewStepTwo();
            }
        ...
        private void Fintran_Review_FormClosed(object sender, FormClosedEventArgs e)
        {
            ReportCompleted();
        }

        public void ReadOnlyButtons(Boolean state)
        {
            this.btnSave_UpdateStepFour.Visible = state;
            this.btnCompletedStepThree.Visible = state;
            this.btnDeleteRecordStepThree.Visible = state;
            this.btnDeleteStepTwo.Visible = state;
            this.btnNewRecordStepThree.Visible = state;
            this.btnNewRecordStepTwo.Visible = state;
            this.btnSave_UpdateStepFour.Visible = state;
            this.btnSave_UpdateStepOne.Visible = state;
            this.btnSave_UpdateStepThree.Visible = state;
            this.btnSaveStepTwo.Visible = state;
        }

        #region Batch Review

        public void loadBatchReviewGrid()
        {
            this.gridViewBatchView.DataSource = dbcon.getBatchReviewInfo();
            this.BatchReviewWindow.Text = "Batch Review - Total Number of Opened Batch is: " + this.gridViewBatchView.RowCount.ToString();
            loadSelectBatch();
        }

        private void documentTabStrip1_Click(object sender, EventArgs e)
        {
            if (this.documentTabStrip1.SelectedTab == this.BatchReviewWindow)
            {
                loadBatchReviewGrid();
            }
        }

        private void gridViewBatchView_DoubleClick(object sender, EventArgs e)
        {
            string batchNum = this.gridViewBatchView.SelectedRows[0].Cells[0].Value.ToString();

            if (MessageBox.Show("Are you sure you want to close Batch Number: " + batchNum + " ?", "Batch Closure Confirmation", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    Boolean result = dbcon.CloseBath(batchNum);
                    if (result)
                    {
                        dbcon.AuditUser(UserInfo.userName, "Batch Colsed - Criteria = Batch Number " + batchNum, 2, DateTime.Now);
                        MessageBox.Show("Batch Number: " + batchNum + " Closed Successfully", "Batch Closed", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        loadBatchReviewGrid();
                    }
                }
                catch (Exception ex)
        ...
        private void gridViewBatchView_CellFormatting(object sender, CellFormattingEventArgs e)
        {
            if (e.CellElement.ColumnInfo.HeaderText == "Date Created")
            {
                e.CellElement.Text = string.Format(CultureInfo.CreateSpecificCulture("en-us"), "{0:D}", e.CellElement.Value);
            }

            if (e.CellElement.ColumnInfo.HeaderText == "Batch Date")
            {
                e.CellElement.Text = string.Format(CultureInfo.CreateSpecificCulture("en-us"), "{0:D}", e.CellElement.Value);
            }
        }

        #endregion Batch Review

        #region Context Strip, Grid Format and Rad Controls
        ...
        private void listAccountTypeStepTwo_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventArgs e)
        {
            ...
            if (this.listAccountTypeStepTwo.Text.ToString().Equals("[Other]"))
            {
                ...
                this.txtAccountTypeStepTwo.Focus();
            }
            else
            {
        ...
        private void chkBoxReportNumberUpdate_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (this.chkBoxReportNumberUpdate.Checked)
            {
                this.btnSearchStepOne.Text = "Update Report Number";
            }
            else
            {
                this.btnSearchStepOne.Text = "Search";
        ...
        private void setBackGroundColourToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (colorDlg.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                this.groupboxStepOne.BackColor = colorDlg.Color;
                this.groupBoxPart2.BackColor = colorDlg.Color;
        ...
        #endregion Context Strip, Grid Format and Rad Controls

        private void ListSelectBatch_SelectedValueChanged(object sender, EventArgs e)
        {
            loadReportNumbers(this.ListSelectBatch.Text.ToString());
        }

        //BATCH GO THROUGH DROPDOWN LIST
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(this.listReportNumber.Text.ToString()) && this.listReportNumber.Text.ToString() != "System.Data.DataRowView")
            {
                this.txtReportNumberStepOne.Text = string.Empty;
                this.txtReportNumberStepOne.Text = this.listReportNumber.Text.ToString();
                this.btnSearchStepOne.PerformClick();
            }
            else
            {
                this.txtReportNumberStepOne.Text = string.Empty;
        ...
        //DELETE ENTIRE REPORT - CASCADE DELETE - ADMIN ONLY FUNCTION
        private void btnDeleteReport_Click(object sender, EventArgs e)
        {
            this.txtReportNumberStepOne.Text = FintranList.ReportID;

            if (!string.IsNullOrWhiteSpace(FintranList.ReportID))
            {
                if (MessageBox.Show("Are you sure you want to delete Report Number " + FintranList.ReportID + " ?", "Delete", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    try
                    {
                        Boolean result = dbcon.DeleteReport(FintranList.ReportID);
                        if (result)
                        {
                            dbcon.AuditUser(UserInfo.userName, "Report Deleted - Criteria = Report ID " + FintranList.ReportID, 2, DateTime.Now);
                            MessageBox.Show("Report " + this.txtReportNumberStepOne.Text.ToString() + " Successfully Deleted");
                            ReportCompleted();
                            this.txtReportNumberStepOne.Text = string.Empty;
                            loadReportNumbers(this.ListSelectBatch.Text.ToString());
                        }
            ...
            {
                MessageBox.Show("Report Number not selected");
            }
        }
